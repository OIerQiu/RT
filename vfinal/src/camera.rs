    ...
    pub fn render(&mut self, world:&HittableList) {
        ...
        for j in 0..self.image_height {
            ...
            for i in 0..self.image_width {
                let mut mpixel_color = Arc::new(Mutex::new(Vector3::new(0.0,0.0,0.0)));
                let mut threads = Vec::new();
                for sample in 0..std::cmp::min(self.samples_per_pixel,28){
                    let wself = Arc::clone(&wself);
                    let wworld = Arc::clone(&wworld);
                    let mut mpixel_color = Arc::clone(&mpixel_color);
                    let render_thread = thread::spawn(move || {
                        let mut ultra = 0;
                        if wself.samples_per_pixel % 28 > sample {
                            ultra = 1;
                        }
                        let mut ppc = Vector3::new(0.0,0.0,0.0);
                        for k in 0..wself.samples_per_pixel / 28 + ultra {
                            let r = wself.get_ray(i,j);
                            ppc += Self::ray_color(&r,wself.max_depth,&wworld);
                        }
                        let r = wself.get_ray(i,j);
                        *mpixel_color.lock().unwrap() += ppc;
                    });
                ...
                let pixel_color = *mpixel_color.lock().unwrap();
